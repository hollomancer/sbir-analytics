...
| Prospective SBIR M&A dated signal study | F1 — what dated observed M&A signals are present in a newly reviewed SBIR firm frame as of 2026-08-29 UTC? | `explorations/sbir_ma_dated_signal_study.ipynb` | Blocked pre-run protocol. Sources acquired privately under operator authorization (amendments 1, 3, 5, 6) and a source-field predicate observed (amendment 7); identity resolution, aggregation, numerical output, and April-result reproduction all remain blocked. Exploratory and non-citable pending human source/privacy/license approval and blinded validation design. |
