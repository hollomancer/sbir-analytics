...
class FiscalNAICSEnricher:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized FiscalNAICSEnricher with {len(self.agency_defaults)} agency mappings"
        )
    ...
    async def _enrich_from_api_uei(
        self, uei: str, award_id: str | None = None
    ) -> NAICSEnrichmentResult | None:
        ...
        try:
            ...
            if result.get("success") and result.get("payload"):
                ...
                if naics_code:
                    ...
                    if award_id:
                        from ..utils.enrichment_freshness import (
                            FreshnessStore,
    ...
    async def _enrich_from_api_duns(
        self, duns: str, award_id: str | None = None
    ) -> NAICSEnrichmentResult | None:
    ...
    def enrich_from_usaspending(
        self, award_row: pd.Series, usaspending_df: pd.DataFrame | None = None
    ) -> NAICSEnrichmentResult | None:
        ...
        if "_usaspending_match_method" in award_row.index and pd.notna(
            award_row["_usaspending_match_method"]
        ):
            # Use existing match
            if "recipient_naics" in award_row.index and pd.notna(award_row["recipient_naics"]):
                ...
                if naics_code:
                    confidence = (
                        0.90 if "exact" in str(award_row["_usaspending_match_method"]) else 0.85
                    )
                    return NAICSEnrichmentResult(
                        naics_code=naics_code,
        ...
        if uei and pd.notna(uei):
            ...
            if not matches.empty and "naics_code" in matches.columns:
                ...
                if not naics_codes.empty:
                    ...
                    most_common_naics = (
                        naics_codes.mode().iloc[0]
                        if not naics_codes.mode().empty
                        else naics_codes.iloc[0]
                    )
    ...
    def enrich_from_usaspending_cage(
        self, award_row: pd.Series, usaspending_df: pd.DataFrame
    ) -> NAICSEnrichmentResult | None:
        ...
        cage_code = (
            award_row.get("CAGE")
            or award_row.get("cage")
            or award_row.get("Cage")
            or award_row.get("company_cage")
            or award_row.get("recipient_cage")
        )
        ...
        for col in cage_columns:
            if col in usaspending_df.columns:
                matches = usaspending_df[
                    usaspending_df[col].astype(str).str.upper().str.strip() == clean_cage
                ]
                if not matches.empty and "naics_code" in matches.columns:
                    naics_codes = matches["naics_code"].dropna()
                    if not naics_codes.empty:
    ...
    def enrich_from_usaspending_contract(
        self, award_row: pd.Series, usaspending_df: pd.DataFrame
    ) -> NAICSEnrichmentResult | None:
        ...
        contract_number = (
            award_row.get("Contract")
            or award_row.get("contract")
            or award_row.get("contract_number")
            or award_row.get("piid")
        )
        ...
        for col in contract_columns:
            if col in usaspending_df.columns:
                matches = usaspending_df[
                    usaspending_df[col].astype(str).str.strip() == contract_str
                ]
                if not matches.empty and "naics_code" in matches.columns:
                    naics_codes = matches["naics_code"].dropna()
                    if not naics_codes.empty:
    ...
    def enrich_from_topic_code(self, award_row: pd.Series) -> NAICSEnrichmentResult | None:
        ...
        # Look for topic code
        topic_code = (
            award_row.get("Topic Code")
            or award_row.get("topic_code")
            or award_row.get("Topic_Code")
            or award_row.get("topic")
        )
    ...
    def enrich_single_award(
        ...
        enrichment_methods = [
            ...
            lambda row: self.enrich_from_usaspending(row, usaspending_df)
            if usaspending_df is not None
            else None,
            lambda row: self.enrich_from_usaspending_cage(row, usaspending_df)
            if usaspending_df is not None
            else None,
            lambda row: self.enrich_from_usaspending_contract(row, usaspending_df)
            if usaspending_df is not None
            else None,
    ...
    def enrich_awards_dataframe(
        self, awards_df: pd.DataFrame, usaspending_df: pd.DataFrame | None = None
    ) -> pd.DataFrame:
        ...
        logger.info(
            f"  Coverage: {naics_coverage:.1%} ({enriched_df['fiscal_naics_code'].notna().sum()}/{total_awards})"
        )
    ...
    def validate_enrichment_quality(self, enriched_df: pd.DataFrame) -> dict[str, Any]:
        ...
        naics_coverage = (
            enriched_df["fiscal_naics_code"].notna().sum() / total_awards if total_awards > 0 else 0
        )
        ...
        quality_results = {
            ...
            "coverage_meets_threshold": naics_coverage
            >= self.quality_thresholds.get("naics_coverage_rate", 0.85),
            "confidence_distribution": {
        ...
        if quality_results["coverage_meets_threshold"]:
            ...
            logger.warning(
                f"NAICS enrichment quality: FAIL (coverage: {naics_coverage:.1%}, threshold: {quality_results['naics_coverage_threshold']:.1%})"
            )
