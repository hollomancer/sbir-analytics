...
class FiscalReturnSummary(BaseModel):
    ...
    payback_period_years: Optional[float] = Field(None, description="Years to recover investment")
    ...
    @classmethod
    def validate_payback_period(cls, v: Optional[float]) -> Optional[float]:
...
class InflationAdjustment(BaseModel):
    ...
    quality_flag: Optional[str] = Field(None, description="Quality flag if adjustment is estimated")
...
class GeographicResolution(BaseModel):
    ...
    # Identifiers
    award_id: str = Field(..., description="SBIR award identifier")
    company_uei: Optional[str] = Field(None, description="Company UEI if available")
    
    # Geographic data
    original_address: Optional[str] = Field(None, description="Original company address")
    resolved_state: str = Field(..., description="Resolved two-letter state code")
    resolved_city: Optional[str] = Field(None, description="Resolved city name")
    resolved_zip: Optional[str] = Field(None, description="Resolved ZIP code")
