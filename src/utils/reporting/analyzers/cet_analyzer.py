...
class CetClassificationAnalyzer(ModuleAnalyzer):
    ...
    def get_key_metrics(self, module_data: dict[str, Any]) -> dict[str, Any]:
        ...
        classification_success = self._calculate_classification_success(
            classified_df, classification_results
        )
    ...
    def generate_insights(self, module_data: dict[str, Any]) -> list[AnalysisInsight]:
        ...
        if classification_rate < self.thresholds["min_classification_rate"]:
            insights.append(
                AnalysisInsight(
                    ...
                    metadata={
                        "current_rate": classification_rate,
                        "threshold": self.thresholds["min_classification_rate"],
                    },
                )
            )
        ...
        taxonomy_coverage = self._calculate_taxonomy_coverage(
            classified_df, module_data.get("taxonomy_data", {})
        )
            ...
            insights.append(
                AnalysisInsight(
                    ...
                    metadata={
                        ...
                        "threshold": self.thresholds["min_taxonomy_coverage"],
                    },
                )
            )
        ...
        if unclassified_rate > self.thresholds["max_unclassified_rate"]:
            insights.append(
                AnalysisInsight(
                    ...
                    metadata={
                        "current_rate": unclassified_rate,
                        "threshold": self.thresholds["max_unclassified_rate"],
                    },
                )
            )
        ...
        if category_dist:
            ...
            imbalance_ratio = (
                max_category_rate / min_category_rate if min_category_rate > 0 else float("inf")
            )
                ...
                insights.append(
                    AnalysisInsight(
                        ...
                        metadata={
                            ...
                            "max_rate": max_category_rate,
                            "min_rate": min_category_rate,
                        },
                    )
                )
        ...
        for col in cet_columns:
            if col in classified_df.columns:
                ...
                # Handle multiple category assignments (JSON or list format)
                    ...
                    for _idx, value in classified_df[col].dropna().items():
    ...
    def _calculate_confidence_distribution(self, classified_df: pd.DataFrame) -> dict[str, Any]:
        ...
        medium_confidence = (
            (confidence_series >= self.confidence_bands["medium"][0])
            & (confidence_series <= self.confidence_bands["medium"][1])
        ).sum()
        low_confidence = (
            (confidence_series >= self.confidence_bands["low"][0])
            & (confidence_series <= self.confidence_bands["low"][1])
        ).sum()
    ...
    def _calculate_taxonomy_coverage(
        self, classified_df: pd.DataFrame, taxonomy_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _calculate_classification_success(
        self, classified_df: pd.DataFrame, classification_results: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for col in classification_columns:
            if col in classified_df.columns:
                if col == "is_classified":
                    # Boolean indicator
                    classified_count = (
                        classified_df[col].sum() if classified_df[col].dtype == bool else 0
                    )
                else:
    ...
    def _calculate_evidence_metrics(self, classified_df: pd.DataFrame) -> dict[str, Any]:
        ...
        if total_evidence_fields > 0:
            ...
            has_any_evidence = classified_df[existing_evidence_columns].notna().any(axis=1).sum()
    ...
    def _calculate_data_hygiene(
        ...
        quality_scores = []
        for _idx, row in classified_df.iterrows():
