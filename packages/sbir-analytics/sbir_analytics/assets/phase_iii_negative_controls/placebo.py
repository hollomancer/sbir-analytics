...
def _cross_firm_assignment(unique_awards: pd.DataFrame, seed: int = PLACEBO_SEED) -> pd.DataFrame:
    """Build one randomized cyclic assignment (not a uniform derangement).

    ``seed`` defaults to the frozen ``PLACEBO_SEED`` so the single materialized
    placebo is unchanged. A caller may pass another seed to draw a further
    independent assignment: one draw gives a direction, and an inferential
    statement about that direction needs a distribution over many.
    """
...
def build_placebo_census_tables(
    ...
    tables = build_placebo_study_tables(pairs, data_cut_date, seed=seed)
