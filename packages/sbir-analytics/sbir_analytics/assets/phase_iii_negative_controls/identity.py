...
This module does not read sources, construct controls, match firms, or run the
census. Source adapters must canonicalize their official award keys before
calling :func:`resolve_award_identities`.
"""
...
def resolve_award_identities(
    ...
    grouped = {
        key: group.drop(columns="_join_key")
...
def reconcile_award_identity_attempts(attempt_audits: pd.DataFrame) -> pd.DataFrame:
