...
The transition detection system aligns awards and contracts against the **21 Critical and Emerging Technology areas** of the NSTC framework. The area list, definitions, and per-area keywords are **not duplicated here** — the single source of truth is [`config/cet/taxonomy.yaml`](../../config/cet/taxonomy.yaml) (version `NSTC-2025Q1`). Refer to that file for the authoritative set of areas and keywords.

This document covers how CET areas are *consumed* as a transition signal (award CET extraction, contract CET inference, and the `cet_alignment` score). For the model-builder perspective — how awards are classified into CET areas with the `ApplicabilityModel` — see [docs/ml/cet-integration.md](../ml/cet-integration.md).

> Note: This document predates the consolidation of CET keyword configuration. Contract-CET inference here uses its own keyword lists under `config/transition/detection.yaml` (see [Configuration](#configuration) below). Where possible, prefer the taxonomy keywords in `config/cet/taxonomy.yaml` as the canonical vocabulary.
