...
- **Status indeterminate for 72.5%** of the cohort after recovery (82.6% on flags alone) — a measurement problem, not necessarily a commercialization failure.
- **Patent lens (new):** 481 SBIR Phase II firms hold B82-classified nanotechnology patents. Half filed their first nanotech application only *after* their first Phase II award (a commercialization-adjacent outcome); the rest filed before or during the award era (a capability marker). The patent data validates the keyword cohort definition, empirically retires the CET proxy, recovers post-award activity evidence for 11%–52% of the firms otherwise counted as "disappeared" (Finding 3), and marks out a 742-award high-confidence core with 23.7% FPDS-coded Phase III (Finding 5).
...
Patent data now brackets this bucket. Of the 651 firms behind these awards, 93 (14%) hold nanotechnology-classified (B82) patents and **69 (11%) filed nanotech patent applications after their first Phase II award** — demonstrably alive and developing nanotech IP while invisible to procurement records. That is the high-precision floor: nanotech firms matching nanotech patents leaves little room for name collision. Extending the match to patents of *any* class, 428 firms (66%) match a patent assignee and 341 (52%) filed after their first award — an upper bound, since exact name matching against the full assignee universe can collide on generic firm names and awaits the same confidence scoring (state, inventor-vs-PI) used for Form D matches. The defensible statement: **between 11% and 52% of the "disappeared" firms provably continued R&D activity after their awards**, using nothing but free public IP data. The no-UEI bucket brackets similarly (12%–37% of 368 firms). Per-firm results: `data/nano_dark_firm_liveness.csv` (`scripts/data/nano_dark_firm_liveness.py`).
...
**2. Observable commercialization is at least one award in four — and even that is a floor, not a rate.** Flag-based channels show 17.4% of awards with a commercialization signal; recovering the awards mislabeled by the FPDS coding gap lifts the observable share to 27.5% (Finding 4). The remaining 72.5% is dominated by measurement failure rather than proven failure: sub-threshold acquisitions and licensing are invisible, and between 11% and 52% of the firms counted as "disappeared" were provably filing patent applications after their awards (Finding 3). Commercialization can be demonstrated for more than one award in four; failure can be demonstrated for almost none.
