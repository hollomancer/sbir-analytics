...
Patent data now quantifies this bucket. Of the 651 firms behind these awards, 428 (66%) match a patent assignee of some class and 341 (52%) filed after their first award. Because exact name matching against the full assignee universe can collide on generic firm names, every match carries a confidence tier built from three corroboration signals (assignee-state agreement, inventor-vs-PI name overlap, name specificity). The result is decisive: **317 of 651 "disappeared" firms (49%) filed patent applications after their first Phase II award at high confidence — and every high-confidence match is corroborated by inventor↔PI name agreement**: the same person who ran the firm's SBIR research appears as an inventor on the matched patents. Half of the population that federal spending data wrote off was demonstrably alive and developing IP, using nothing but free public data. The no-UEI bucket behaves the same way (126 of 368, 34%, at high confidence). Per-firm results with tiers: `data/nano_dark_firm_liveness.csv` (`scripts/data/nano_dark_firm_liveness.py`).
...
**2. Observable commercialization is at least one award in four — and even that is a floor, not a rate.** Flag-based channels show 17.4% of awards with a commercialization signal; recovering the awards mislabeled by the FPDS coding gap and resolving no-UEI identities lift the observable share to 28.1% (Findings 3–4). The remaining 71.9% is dominated by measurement failure rather than proven failure: sub-threshold acquisitions and licensing are invisible, and 49% of the firms counted as "disappeared" were provably filing patent applications after their awards, at high confidence (Finding 3). Commercialization can be demonstrated for more than one award in four; failure can be demonstrated for almost none.
