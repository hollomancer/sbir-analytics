...
- **Neo4j smoke tests** (optional) via `scripts/data/run_neo4j_sbir_load.py` loads validated awards into a Neo4j cloud instance, creates award nodes (`:FinancialTransaction {transaction_type: "AWARD"}`) and `:Organization` nodes linked by `:RECIPIENT_OF` relationships, and runs Cypher smoke checks via `scripts/data/run_neo4j_smoke_checks.py` to verify graph structure. The database is reset before each run via `scripts/data/reset_neo4j_sbir.py` to ensure clean state. See [Neo4j Setup](../getting-started/README.md) for configuration.
...
The workflow connects to a Neo4j cloud instance (if configured), resets it to a clean state, loads validated SBIR awards using the `neo4j_sbir_awards` asset, and runs smoke checks that verify:

- Award node count > 0 (`:FinancialTransaction {transaction_type: "AWARD"}`)
- Organization node count > 0 (`:Organization`)
- `:RECIPIENT_OF` relationship count > 0
- Sample award properties are populated correctly
- Award→Organization connectivity (`(:FinancialTransaction)-[:RECIPIENT_OF]->(:Organization)`) is established
