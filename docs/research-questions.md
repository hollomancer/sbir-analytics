...
**CET spine.** The organizing spine is the repo's **21-area `NSTC-2025Q1`** taxonomy defined in `config/cet/taxonomy.yaml`; `packages/sbir-ml/sbir_ml/ml/config/taxonomy_loader.py` expects exactly 21 areas and logs a warning on any mismatch (a soft check, not a hard-failing validation). This is *not* the external 18-area Feb-2024 NSTC Critical and Emerging Technologies list,[^cet-count] nor DoD's critical-technology list — both are narrower external frameworks, and the repo's 21-area set already blends NSTC CET areas with several DoD critical-technology areas (Hypersonics, Directed Energy, Advanced Gas Turbine Engine Technologies, Integrated Network Systems-of-Systems). The DoD list is itself in flux: on November 17, 2025 the Under Secretary for Research and Engineering cut it from the 14 areas of the **DoD-14 (2023 NDSTS, superseded Nov. 17, 2025)** to six — Applied Artificial Intelligence (AAI); Biomanufacturing (BIO); Contested Logistics Technologies (LOG); Quantum and Battlefield Information Dominance (Q-BID); Scaled Directed Energy (SCADE); Scaled Hypersonics (SHY) (war.gov press release, Nov. 17, 2025; cto.mil/cta). **Crosswalk note:** for DoD-facing outputs each CET area should also carry a **DoD-6** tag against that current list and, where a defense-industrial-priority mapping is wanted, an NDIS tag keyed to the 2024 National Defense Industrial Strategy's four strategic priorities (resilient supply chains, workforce readiness, flexible acquisition, economic deterrence; released Jan. 11, 2024) — or, for finer granularity, its ten systemic challenges (CRS IN12310) or the NDIS Implementation Plan's six cross-cutting initiatives (Oct. 2024, CRS IN12459) — so results speak to both NSTC and DoD audiences. (Two other, divergent CET taxonomies exist in code — a 10-area transition-system set and a 19-area reporting-analyzer set — and are not yet reconciled to the canonical 21; see Maintenance.)

[^cet-count]: The "18" here counts top-level NSTC Critical and Emerging Technology *areas*, not the number of contributing departments/agencies — two figures that are easily conflated. The CET area count has trended down across successive NSTC updates: 20 areas (Oct. 2020) → 19 (Feb. 2022) → 18 (Feb. 2024).

**Statutory grounding.** Axis B maps to the risk-based due-diligence factors of the SBIR/STTR reauthorization, **Pub. L. 119-83** (signed April 13, 2026), and its eight restricted-entity screening lists — the UFLPA Entity List, the Non-SDN Chinese Military-Industrial Complex Companies (NS-CMIC) List, the Section 889 Prohibition List, the 1260H list, the Military End-User (MEU) List, the BIS Entity List, the FCC Covered List, and the CBP WRO/Findings List [L26]. Axis A maps to the same law's Strategic Breakthrough Award/Allocation (15 U.S.C. 638(ff)(3) and 638(aa)(6), added by Pub. L. 119-83 §3 — a mechanism that ceases to have effect September 30, 2031 per §3(d)) and its Phase III provisions. (Statute linked, not reproduced.)
...
- Did this SBIR-funded research result in a federal contract? — [transition/overview.md](transition/overview.md). Baselines: NASEM DoD [L1][L2]; Link & Scott ~50% commercialization probability [L12]; Gans & Stern on the determinants of when smaller-firm SBIR research bears fruit [L30]; NASEM program reviews [L3][L4][L6]. *(deps: ER, ID)*
...
- What public-sector commercialization success factors distinguish Phase III awardees — particularly in the Department of the Navy, the highest-volume Phase III source? Benchmark against Rovito, Kamp & Etemadi's Navy Phase III analysis [L31]. *(deps: ER, ID, transitions)*
...
- Which Phase II awardees subject to §638(qq)(3) Increased Performance Standards meet the **statutory Commercialization Benchmark** (sales + private investment over the 10-FY covered period ÷ SBIR funding ≥ specified ratio)? 15 U.S.C. 638(qq), as added by Pub. L. 117-183 §8 and amended by Pub. L. 119-83 §10(f) (a sunset extension to Sept. 30, 2031 in §638(qq)(3)(I) — the benchmark formula, ratio, covered period, and covered population are unchanged). Implementation on main: `scripts/run_benchmark.py` (evaluate / sensitivity / company-level CLI) backed by `sbir_etl/models/benchmark_models.py`, with tests in `tests/unit/test_benchmark_evaluator.py`. Additional per-firm audit infrastructure and a more comprehensive methodology doc exist as local-only / uncommitted work — see "Output products" section below for the in-progress status. *(deps: ER, ID, transitions, SEC EDGAR)*
- Which firms sit in the extreme tail of the award-count distribution ("SBIR mills") — high cumulative SBIR awards but low transition/commercialization — and how would the new per-company proposal caps bite there? Statutory anchor: per-company proposal limits at 15 U.S.C. 638(aaa), added by Pub. L. 119-83 §4, effective FY2027 (per-fiscal-year, per-solicitation, or per-topic limits; time-sensitive-topic waivers capped at 5% of an agency's topics). GAO found increased performance standards would likely affect few multiple-award firms [L32]; Lanahan, Joshi & Johnson characterize this extreme tail of the award distribution [L27]. *(deps: ER, ID, transitions)*
...
- Net job creation: do SBIR recipients add more employees than comparable non-recipients, or fewer? A discovery-class, contestable result worth reproducing — Lanahan, Joshi & Johnson find recipients hire *fewer* employees than matched non-recipients, a net/counterfactual measure distinct from the gross IMPLAN employment impacts above [L27]. *(deps: ER, ID)*
- How do fiscal returns stratify by state and NAICS sector? *(deps: ER, NAICS)*
- Do states with their own SBIR match/augmentation programs show measurably different award density or follow-on outcomes — i.e., does the multilevel federal-plus-state policy mix shift the program's geographic distribution? Lanahan & Feldman on state augmentation policy [L29]; the canonical distributional finding remains Lerner's concentration in VC-rich zip codes [L10]. *(deps: ER, ID)*
...
- Does SBIR funding crowd in or crowd out subsequent private capital? **Target: reproduce or extend Howell's finding that an early-stage DOE SBIR grant roughly doubles the probability of subsequent VC** [L11]. An alternative identification strategy: Lanahan & Feldman exploit the Kentucky and North Carolina SBIR state-match programs to approximate exogenous variation in R&D, complementing Howell's regression-discontinuity design [L28]. Compare against Kortum & Lerner [L24] on VC's contribution to innovation. *(deps: ER, ID, SEC EDGAR)*
...
- **[L27]** Lanahan, L., Joshi, A.M., & Johnson, E. (2021). "Do public R&D subsidies produce jobs? Evidence from the SBIR/STTR program." *Research Policy* 50(7), 104286. Finding: award recipients hire fewer employees than matched non-recipients — a contestable, discovery-class net-employment result — and a study of the extreme tail of the award distribution ("SBIR mills").
- **[L28]** Lanahan, L. & Feldman, M.P. (2018). "Approximating exogenous variation in R&D: Evidence from the Kentucky and North Carolina SBIR state match programs." *Review of Economics and Statistics* 100(4), 740–752. State-match natural experiment as a causal identification strategy for SBIR effects, complementary to Howell's regression-discontinuity design [L11].
- **[L29]** Lanahan, L. & Feldman, M.P. (2015). "Multilevel innovation policy mix: A closer look at state policies that augment the federal SBIR program." *Research Policy* 44(7), 1387–1402. State augmentation/match policies and their effect on the geographic distribution of SBIR activity.
- **[L30]** Gans, J. & Stern, S. (2003). "When does funding research by smaller firms bear fruit? Evidence from the SBIR program." *Economics of Innovation and New Technology* 12(4), 361–384. Determinants of SBIR commercialization success; complements Link & Scott [L12].
- **[L31]** Rovito, S.M., Kamp, J., & Etemadi, A. (2025). "Exploring Department of the Navy SBIR Phase III awards and corresponding public sector commercialization success factors." *Journal of Technology Transfer* 50(4), 1363–1395. Navy Phase III commercialization success factors — the most directly relevant study to the Phase III transition work.
...
- **[L32]** GAO-24-106398 (2024). *Small Business Research Programs: Increased Performance Standards Likely Affect Few Businesses Receiving Multiple Awards* (Mar. 29, 2024). Anchors the SBIR-mills / per-company proposal-caps discussion; distinct from GAO-24-107036 [L14], whose title was previously confused with it. <https://www.gao.gov/products/gao-24-106398>
...
- **[L26]** Pub. L. 119-83 — **Small Business Innovation and Economic Security Act** (SBIR/STTR reauthorization; enrolled as S. 3971, signed April 13, 2026). Establishes the risk-based due-diligence factors and the eight restricted-entity screening lists (UFLPA Entity List; NS-CMIC List; Section 889 Prohibition List; 1260H list; Military End-User List; BIS Entity List; FCC Covered List; CBP WRO/Findings List) that ground Axis B; the Strategic Breakthrough Award/Allocation at 15 U.S.C. 638(ff)(3) and 638(aa)(6) that grounds Axis A (§3, terminating Sept. 30, 2031 per §3(d)); the per-company proposal caps at 15 U.S.C. 638(aaa) (§4, effective FY2027); and the §638(qq)(3)(I) Commercialization Benchmark sunset extension to Sept. 30, 2031 (§10(f)). <https://www.congress.gov/bill/119th-congress/senate-bill/3971>

---

## Maintenance

**Last reviewed:** 2026-06-25 — incorporated verified statutory, framework, and literature corrections: re-cited the Commercialization Benchmark to 15 U.S.C. 638(qq) (added by Pub. L. 117-183 §8, amended by Pub. L. 119-83 §10(f)) and added the Pub. L. 119-83 statutory anchors for the Strategic Breakthrough Award (15 U.S.C. 638(ff)(3)/(aa)(6), §3, terminating 2031) and per-company proposal caps (15 U.S.C. 638(aaa), §4, effective FY2027); corrected the DoD critical-technology list from 14 to the current 6 (USD(R&E), Nov. 17, 2025); fixed the NSTC CET-count footnote (20→19→18 trend) and removed the unverified "NDIS-8" set in favor of the 2024 NDIS's four strategic priorities; and integrated previously-researched literature [L27]–[L32]. Prior review 2026-06-21 — staleness audit verified all PR refs, branch tags, spec/doc links, and architectural notes against current `main`. Three corrections shipped via PR #335 (F3 supersession by #321, E4 spec landed via #277, A4 M&A pipeline architecture note). A follow-on PR #341 (currently open) proposes updating the published Form D fundraising doc with bootstrap CIs and PIF cross-link audit findings.
