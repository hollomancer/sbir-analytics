...
- Which awards/solicitations to entity X plausibly derive from firm Y's Phase II work (Y ≠ X)? Ranked **bypass leads** for human review — never violation findings — gated on a text-match benchmark: [../specs/product-2-bypass-leads/](../specs/product-2-bypass-leads/), [../specs/phase3-match-benchmark/](../specs/phase3-match-benchmark/). Benchmark result: separable only where FPDS descriptions are substantive (ModernBERT AUC 0.82 for ≥100-char descriptions; ≈ chance below). **[Partial — viable only in the well-described stratum; upper bound, needs realistic-pool precision@k]** *(deps: ER, ID, transitions)* *(branch: claude/phase3-detection-phase0)*
