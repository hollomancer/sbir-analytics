...
because it is labeled as unapproved evidence and nothing downstream depends on it. In
...
Artifacts that can support approved claims outside this repository.
...
currently `reproducible`, not approved: its study manifest keeps claims
unapproved until complete audit tables and post-write checks have passed.
...
2. **A result is a number with an interval, and it travels.** `validated` means
   ...
   `approved` additionally requires that it was and that one pinned final review
   approves the claim boundary. A fair test the method failed
...
| | |
|---|---|
| **Contract** | Labeled as unapproved evidence. That is the whole contract. |
...
not a fifth tier. An operated artifact remains unapproved evidence and must
emit the legacy `citable: false` metadata on anything that leaves the repository —
...
exploratory may import it, and its numbers stay unapproved no matter how well
tested it is.
...
created only when a research question needs the number to carry approved evidentiary
...
|---|---|
| `Approved evidence` | `evidence_status: approved` |
| `Validated` | `validated` with `validation_result.threshold_met: true`, or `approved` |
