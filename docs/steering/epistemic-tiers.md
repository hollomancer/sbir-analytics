...
- Active specs declare a target tier in `requirements.md`;
  ...
  invalid declarations in CI. Specs that declare `evidence` must also carry
  paperwork for SHA-256 freeze enforcement (a recorded digest or explicit
  raw-byte freeze language in `amendments.md`) plus a `**Declared estimand:**`
  field. That gate is paperwork-only; it does not prove runtime SHA or
  blocking asset-check enforcement.
