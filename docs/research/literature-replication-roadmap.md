...
The maintainer activates this roadmap by selecting the first tranche. Until then, use it as
...
Before work starts, check [`specs/status.md`](../../specs/status.md). That registry controls whether
a named spec is active, gated, or limited to a lower tier.
...
- Do not implement the validation spec until its registry tier authorizes that work.
...
fan-out checks pass, reviewed classifications and disagreements are recorded, and the NASEM
comparison has a row-level audit trail.
...
- Apply the suppression rules in the registered
  [`patent-cost-spillover` contract](../../specs/patent-cost-spillover/requirements.md).
...
- Do not treat inferred follow-on work as Phase III. Run the hand-label validation only after its
  registry gate opens.
...
5. Create one study manifest per replication at `exploratory`. Promote it to `reproducible` when
   its declared inputs and implementation can be rerun. Record `reproduced`, `diverged with
   explanation`, or `blocked` separately.
