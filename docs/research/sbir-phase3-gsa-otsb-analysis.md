...
- **GSA accounts for 27.2% of Phase III dollars ($1.86B of $6.85B keyword-discoverable) and 10.1% of contracts (204 of 2,013) across FY2008–FY2026.** GSA is the *procurement vehicle*, not the funder. GSA funds no Phase III work. DoD pays for 96% of GSA-awarded Phase III.
...
- **GSA Phase III was near zero before FY2014.** It peaked at $508M in FY2021 (64% of all Phase III dollars that year). It then fell to $109M in FY2024 (15% share). **Partial channel shift, partial contraction:** about 40% of the GSA-era dollars came back as direct-DoD work. The other ~60% left keyword-discoverable Phase III. Combined GSA plus direct-DoD Phase III fell 34% from the FY2023 peak ($764M) to FY2025 ($506M).
- **14.7% of Phase III dollars ($1.01B) went to Other Than Small Business (OTSB) recipients**, 7.0% of contracts. **The SBIR ecosystem explains 97% of genuine §638(r) OTSB Phase III dollars.** These dollars come from direct ex-SBIR firms (~87%), corporate spinouts of P1/P2 awardees (Sierra Space ← Sierra Nevada Corp, ~7%), or large primes that team with SBIR subs (~3%). The hard-to-explain residual is ~$25M (~3%), mostly defensible production-scaling. The OTSB label is a poor "program leakage" signal, because the SBIR ecosystem produces the OTSB outcome by design.
- **Only 55.7% of keyword-discoverable Phase III dollars ($3.82B of $6.86B) are §638(r) sole-source.** This is much lower than the Tier 1 OTSB-only puzzle-case investigation suggested. The other 44.3% is competitive procurement, set-asides, IDIQ task orders, or simplified acquisitions. Their descriptions reference Phase III work but do not claim §638(r) authority. **GSA Phase III is 99% §638(r) sole-source; direct-DoD Phase III is only 41%.** GSA's share of the strict §638(r) universe is **48.2%** (vs. 27.2% of the keyword universe). The keyword view therefore *understates* how much FEDSIM controls the actual §638(r) channel.

## What "Phase III" means in this dataset

Phase III is a contracting status, not a separate SBIR award. Under [15 USC §638(r)](https://uscode.house.gov/view.xhtml?req=granuleid:USC-prelim-title15-section638), any federal agency may award a sole-source contract to an SBIR/STTR firm. The contract lets the firm develop, produce, or commercialize technology from an earlier Phase I or Phase II SBIR award. That contract is "Phase III." Phase III is not centrally tracked. SBIR.gov bulk data covers only Phase I (151,426 rows) and Phase II (68,075 rows). It has zero Phase III rows. The only authoritative source for Phase III contracts is FPDS/USAspending. There, the contracting officer sets Phase III status. They write "SBIR Phase III" (or a variant) into the contract description.

That tagging is unreliable. [GAO-24-107036](https://www.gao.gov/products/gao-24-107036) documents a ~30% undercount of Phase III contracts in DoD historical FPDS tagging. The cause: contracting officers do not always include Phase III markers in descriptions. This analysis works within that limit. Read the dataset as the **keyword-discoverable** Phase III universe, not the true population.

## Methodology

Two scripts build the cached USAspending Phase III set in this repo:
...
**Coverage gain vs. the prior single-keyword fetch:** +20% rows (1,671 → 2,013) and +14% dollars ($6.00B → $6.85B). FY2007 returns HTTP 500 from USAspending — that is the API's reliable depth limit. 99.7% of rows enriched successfully with size data.

### What the methodology does *not* fix

Six keyword variants caught more rows, but they still miss some contracts. Phase III contracts whose descriptions use **no** SBIR/Phase-III markers stay invisible. No authoritative join exists, because SBIR.gov does not track Phase III. **The cohorts under-count direct-DoD contracts more than GSA-awarded contracts.** FEDSIM templated SOWs routinely include the "SBIR Phase III" phrase, but DoD program offices vary widely. The GSA share reported here is therefore **likely modestly overstated** against ground truth. The true GSA dollar share is probably 20–27%, not the surface-level 27.2%.

The methodology also keeps some **non-§638(r) contracts whose descriptions contain "SBIR Phase III"** as program-context language. The Tier 1 puzzle-case investigation (see below) showed that ~$45M of OTSB rows fit this pattern. These are task orders against multiple-award IDIQs, competitively-procured contracts, small-business set-asides, and simplified-acquisition contracts. Their descriptions reference Phase III work, but their procurement authority is not §638(r) sole-source. To fix this, fetch FPDS `extent_competed`, `solicitation_procedures`, and `other_than_full_and_open_competition_description` at scrape time, then filter on those. That is a small change to the universe builder. This document notes the contamination in the OTSB sub-population, but does not quantify it across the full $6.85B universe.
