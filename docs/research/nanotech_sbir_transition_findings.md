...
**What the numbers imply for private capital:** Among the 272 temporally-filtered Form D matches (135 unique firms), the per-firm median raise was approximately $10.6 million, with the 90th percentile approximately $66 million. The median lag from Phase II end to first offering was approximately 2.7 years — consistent with a product development gap between government-funded prototype and investor-ready asset. This lag has policy implications: evaluation windows shorter than three to four years will systematically undercount NSF and NIH commercialization activity.
...
**Temporal lags.** The median time from the firm's earliest nanotech Phase II award to confirmed acquisition across the 10 firms with known acquisition dates is approximately 8.5 years. The longest is Physical Optics Corporation, where a Phase II award in 1990 preceded Mercury Systems's acquisition of the firm in 2020 — a 30-year span; EKOS Corporation (1998 Phase II to 2010 acquisition, 12 years) is closer to the middle of the range than the extreme. This reinforces the evaluation window finding from Finding 1: outcome windows of two or three years will miss the majority of acquisition-pathway commercialization events.
...
**The headline finding: these firms mostly did not vanish — the federal data did.** Checking public records outside federal award systems — patents, trademarks, corporate name-change filings, subcontracts to major primes, clinical trials, and FDA clearances — shows that roughly two in three of the 651 "disappeared" firms left a detectable trace of continued activity, and a best statistical estimate of the broader dark population (including the no-UEI firms) suggests the true share may run closer to four in five.

What these firms were actually doing, where visible, falls into a few recognizable patterns:

- **Still building the technology.** Half of the disappeared firms filed a new patent application after their award ended, with the original research team still listed as inventors.
- **Going to market quietly.** A third filed a trademark for a real product — a wound-dressing brand, a diagnostics platform, a lidar-system line — none of it visible in any federal contracting database.
- **Absorbed into a prime's supply chain.** Close to one in five delivered technology as a subcontractor to firms like Northrop Grumman, Lockheed Martin, and SRI International — $3.2B in aggregate post-award subcontract volume, and for roughly one in six of these firms, cumulative subaward revenue already exceeds everything the government ever invested in them. A legitimate SBIR success invisible to federal metrics by design, since a subcontractor never appears as the prime award's recipient of record.
- **Acquired, just under a different name.** A smaller number are confirmed successes hiding behind a rename or merger — WaveBand Corporation's absorption into the defense prime Sierra Nevada Corporation is one example, invisible to every method this analysis used because Sierra Nevada is privately held and never files the disclosures those methods depend on.

We also tried the opposite kind of check — patents a firm let lapse for non-payment, as a proxy for having gone out of business. It mostly did not work: letting old patents lapse turned out to be routine behavior even for companies that are demonstrably still active elsewhere. Only a small subset (17 firms) showed a fully-lapsed portfolio *and* no activity anywhere else — real but modest corroboration, not the shortcut to "confirmed dissolved" it looked like it might be. That failure is itself informative: distinguishing "quietly successful" from "actually defunct" from public records alone has real limits. Closing the remaining gap needs either state corporate-dissolution records or direct outreach to firms, not more clever data matching — both are scoped as next steps.

The 22.7% no-UEI figure is similarly less permanent than it looks: searching USAspending by firm name alone, with no new infrastructure, resolves a candidate identity for 43% of these firms.

Full instrument-by-instrument methodology, confidence tiers, and reproducible code are in Methodological Notes.
...
The measurement lesson is itself a finding about commercialization: the phenomenon is only as visible as the instruments observing it. Procurement data sees the defense half, SEC data sees the venture half, patent data sees the IP half — and each half looks like failure through the other's lens. Statistically, each new instrument mostly finds new firms rather than confirming ones other instruments already found — so no number of positive checks can ever prove how much commercialization remains completely unseen, only keep raising the known floor (Finding 3). The dark majority is not one undifferentiated unknown: part of it is a coherent, still-active subpopulation that enough instruments would eventually illuminate, and part of it is a residual that nothing tried here can reach. Only the second part needs the survey — and even a plausible-sounding way to find it (patents a firm let lapse, as a proxy for having failed) needed the same skepticism as any positive result before it could be trusted, and mostly did not hold up (Finding 3).
...
**2. Evaluation windows need to match commercialization timelines — per pathway, not overall.** The channels run at different speeds: dateable procurement follow-ons arrive fastest (61% of first signals within two years of Phase II end — the federal customer already exists), private raises lag a median of 2.7 years, and prime acquisitions run a median of roughly 8.5 years. A time-to-first-signal survival analysis (`data/analysis/nano_time_to_signal_km.png`) shows roughly 39% of eventually-dateable signals arrive after year two. Annual or biennial assessments therefore capture most procurement transitions while systematically undercounting NIH/NSF private-pathway and acquisition outcomes; pathway-specific windows (two years for procurement, four to five for private capital, seven-plus for exits) fit the data better than any single window.
...
**6. The "disappeared" population needs a triage ladder, not a blind survey.** The 651 procurement-dark firms and 368 no-UEI firms are not one undifferentiated unknown — they decompose into distinct problems needing distinct fixes (full breakdown in `specs/dark-majority-resolution/`): mislabeled awards are recoverable now from contract-level federal data; no-UEI awards need retroactive entity resolution; too-recent awards need survival-analysis framing, not failure counts. For what remains genuinely dark, cheap public-record checks — patents, trademarks, subcontracts, corporate name changes, sector registries — already do most of the work at effectively zero marginal cost, lighting up roughly two-thirds of the residual. What is left after that — our best estimate is somewhere between one-fifth and two-fifths of the truly dark population — is where a targeted survey belongs: on the specific firms these public-record checks could not reach, not a random sample of the whole group.
...
- **Cohort definition:** Keyword matching over text fields will miss nanotech awards where the abstract does not use standard terminology. The published reference's implied universe may be substantially larger.
...
- **External reference figures:** The agency-by-year figures used in the underlying reconciliation analysis are approximate public summary values transcribed from a published federal budget supplement. The source's own methodology for identifying nanotech SBIR awards is not published.
...
- **Maintenance-fee lapses:** Signal is an `EXP.` (expired for non-payment) event with no later `EXPX` (reinstated) event, from the single latest cumulative USPTO PTMNFEE2 snapshot (weekly, covers 1981–present as of 2026-07-06). Scoped to the 582 dark firms already matched to patents at high confidence in the liveness check (§ this section, patents paragraph) — this reuses that verified name-match rather than re-deriving it. Firm-level "portfolio dormant" requires ≥80% of fee-checkpoint-eligible patents (grant date ≥3.5 years before data currency) lapsed, with ≥2 such patents; the threshold was checked at 90% and 100% and the result (most flagged firms are contradicted by other evidence) does not meaningfully change. This channel supplies no positive evidence and is not counted toward the "illuminated" tally anywhere in this report — it only helps stratify the residual.
...
Full methodology, term lists, confidence tags, and reproducible code are in `docs/nano-phase3-methodology.md` and `scripts/data/build_nano_cohort.py`.
