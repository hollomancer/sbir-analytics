...
- [ ] 2.1 Create `scripts/docker/wait-for-service.sh` that blocks until `tcp://host:port` is reachable and use it before starting Dagster services.
...
- [ ] 4.1 Create `.env.example` describing required variables (`NEO4J_URI`, `NEO4J_USER`, `NEO4J_PASSWORD`, `SBIR_ETL__...`) and add `.env` to `.gitignore`.
...
- [ ] 4.4 Ensure compose files and entrypoints only reference `${VARIABLE}` sourced from `.env` or exported environment (no hardcoded credentials).
...
- [ ] 6.1 Create `docs/deployment/containerization.md` covering architecture diagrams, compose profiles, data volume expectations, troubleshooting, and registry workflow.
