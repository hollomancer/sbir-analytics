...
The system SHALL quantify and report the achievable SBIR↔USAspending join rate using the staged Postgres subset before enabling enrichment jobs.

#### Scenario: Measure join coverage vs. 70% target
- **WHEN** the profiled USAspending snapshot has been staged
- **THEN** running `poetry run assess_usaspending_match_rate --sbir-sample data/raw/sbir/sample.csv`
...
- **WHEN** the profiling workflow completes
