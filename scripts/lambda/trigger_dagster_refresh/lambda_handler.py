...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        deployment = event.get("dagster_cloud_deployment") or os.environ.get(
            "DAGSTER_CLOUD_DEPLOYMENT", "prod"
        )
        job_name = event.get("job_name", "sbir_weekly_refresh_job")
        secret_name = event.get("dagster_cloud_secret_name") or os.environ.get(
            "DAGSTER_CLOUD_SECRET_NAME", "sbir-analytics/dagster-cloud-api-token"
        )

        if not org:
            raise ValueError(
                "dagster_cloud_org required in event or DAGSTER_CLOUD_ORG environment variable"
            )
