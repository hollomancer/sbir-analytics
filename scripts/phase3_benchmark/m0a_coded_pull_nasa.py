...
Sharded by fiscal year (SIGNED_DATE), cached under data/raw/fpds/m0a_coded_nasa/. Dedups to award grain
via the compound key. Output: data/derived/m0a_coded_nasa.parquet (the coded Phase III universe)."""
import re, os, time, json, urllib.request, urllib.parse
import pandas as pd

REPO = "/Users/hollomancer/projects/sbir-analytics"
CACHE = f"{REPO}/data/raw/fpds/m0a_coded_nasa"; os.makedirs(CACHE, exist_ok=True)
...
for code in ("SR3", "ST3"):
    for fy in FYS:
        q = f"RESEARCH:{code} DEPARTMENT_ID:8000 SIGNED_DATE:[{fy-1}/10/01,{fy}/09/30]"
        start = 0
        while True:
...
df.drop(columns="_d").to_parquet(f"{REPO}/data/derived/m0a_coded_nasa_txn.parquet")
df_award.to_parquet(f"{REPO}/data/derived/m0a_coded_nasa.parquet")
