...
def per_firm_auc(
    query_texts: list[str], target_texts: list[str], neg_indices: dict[int, np.ndarray]
) -> np.ndarray:
...
def auc_by_length_decile(
    aucs: np.ndarray, lengths: np.ndarray, n_bins: int = 10
) -> list[dict[str, float]]:
    ...
    for b in sorted(set(bins)):
        ...
        rows.append(
            {
                "decile": int(b),
                "len_lo": int(lengths[mask].min()),
                "len_hi": int(lengths[mask].max()),
                "mean_auc": float(aucs[mask].mean()),
                "n": int(mask.sum()),
            }
        )
    return rows
...
def main(argv: list[str] | None = None) -> int:
    ...
    q_abstract = (
        ...
        .apply(lambda s: " ".join(x for x in s if x)[:9000])
        .to_dict()
    )
    ...
    aucs = per_firm_auc(
        [q_abstract[u] for u in firms], [t_desc[u] for u in firms], random_negatives(len(firms), 25)
    )
    print(
        f"AUC by TARGET-description-length decile (rich/rich, TF-IDF, random neg, n={len(firms)}):\n"
    )
