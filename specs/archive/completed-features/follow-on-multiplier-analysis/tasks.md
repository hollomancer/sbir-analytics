# Follow-on Funding Multiplier Analysis — Tasks

## Phase 1: Core Multiplier Computation

- [ ] 1.1 Create `src/tools/mission_b/follow_on_multiplier.py` with `FollowOnMultiplierCalculator` class
- [ ] 1.2 Implement SBIR/STTR obligation separation using FPDS product-or-service codes and contract type flags
- [ ] 1.3 Implement firm-level multiplier computation (non-SBIR obligations / SBIR obligations per vendor)
- [ ] 1.4 Implement aggregate multiplier computation (sum non-SBIR / sum SBIR across all vendors)
- [ ] 1.5 Add entity resolution coverage reporting (match rate, unresolved vendors)

## Phase 2: Cohort Stratification

- [ ] 2.1 Create `CohortStratifier` in `src/tools/mission_b/follow_on_multiplier.py`
...
- [ ] 2.6 Implement time-series decomposition (multiplier by fiscal year, trend detection)
...
- [ ] 4.1 Create `AgencyComparator` for running multiplier computation across agencies
- [ ] 4.2 Implement DOE follow-on funding multiplier (contracts + grants — DOE uses both)
- [ ] 4.3 Implement cross-agency comparison output (table + visualization data)

## Phase 5: Testing and Integration

- [ ] 5.1 Create unit tests for multiplier computation with synthetic FPDS data
- [ ] 5.2 Create integration test with sample vendor universe
- [ ] 5.3 Create Dagster asset wrapper for pipeline execution
- [ ] 5.4 Add follow-on funding multiplier to CLI reporting commands
