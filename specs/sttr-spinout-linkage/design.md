...
| 2 | `SPINOUT_T2` | A **fuzzy** positive in one dimension (`D2.fuzzy_person` above the similarity cutoff and `generic_token_guard` passes, **or** `D5.spinout_phrase`) **AND** independent corroboration from a **second** dimension (`D3`, `D4.form_d_officer_ri_affiliated`, or `D5`). Two independent weak signals. |
