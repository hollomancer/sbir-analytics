...
- **`ma-discovery-integration` — Active (bounded slice).** M&A recall was
  selected as a research priority on 2026-09-06 (inventory anchors A4 / F2).
  The active scope is issue #446 steps 2–3: the #371 toolkit at
  `sbir_etl/enrichers/ma_discovery/`, a real Tavily client (Brave optional)
  behind config with a fail-closed `none` default and opt-in `mock`, and a
  fixture-only extractor eval (keyword vs structured LLM prompt; exploratory /
  non-citable; the LLM path lives in `llm_extractor.py` at exploratory tier).
  The orchestrator still uses the keyword verifier. The C3 collision policy,
  the `MAEvent.confidence` rewrite, and any capital-events join stay out of
  scope until a labeled Form-D-missing sample exists; candidate discovery
  remains non-citable per `requirements.md`.
