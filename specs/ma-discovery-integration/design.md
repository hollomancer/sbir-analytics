...
|---|---|---|
| Search backend (Tavily / Brave / Bing / Serper) | **Tavily** is the default production client; Brave is a second same-shape backend. Runtime default is `none` and fails closed; `mock` is opt-in via `--search-backend mock`. Comparison: [`search-backends.md`](search-backends.md) (exploratory, non-citable — not a recall benchmark). | Comparison note landed; revisit only if the sample run shows snippet quality is the bottleneck |
...
3. **Implement a real `SearchTool`** against the chosen backend, with config + credentials in `.env.example` and `OTConsortiumConfig`-style schema entry. Done (issue #446 step 2): Tavily as the production backend, optional Brave; config default `none` fails closed, `mock` opt-in only.
