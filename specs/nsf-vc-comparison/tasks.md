...
- [ ] 1.1 Add `NSFCohortBuilder` under
  `packages/sbir-analytics/sbir_analytics/assets/nsf_vc/`. Filter award
  universe to ALN ∈ {47.041, 47.084}, stratify by vintage (5-yr buckets) +
  phase. Verify: cohort sizes match SBA annual report [L18] NSF totals
  within 5%.
- [ ] 1.2 Add `OutcomeMetricsCalculator` that reuses the existing transition
  detector (`packages/sbir-ml/sbir_ml/transition/`) and PATLINK. Emits
  Wilson-CI-bounded rates per stratum. M&A exit rate consumes #286's
  `data/sbir_ma_events.jsonl` directly (post-rebase). Verify: re-running
  on the leverage-ratio fixture set reproduces transition rates within
  tolerance.
...
- [ ] 2.1 Add `NSFAwardeeFilter` — apply ALN 47.041 / 47.084 to #286's
  resolved SBIR-CIK set produced by `sec_edgar_enrichment`. Output:
  NSF-CIK set + NSF-UEI set, persisted as parquet.
...
- [ ] 2.3 Add `CohortMatcher` — coarsened-exact matching on (vintage,
  ...
  matching ratio (NSF firm : k matched controls) in the output.
- [ ] 2.4 Add `MatchedCohortOutcomes` — join both cohorts to FPDS
  ...
  where applicable; phase-graduation and survival metrics are NSF-only
  (control N/A).
...
- [ ] 2.6 Wire Phase 2 as a Dagster asset
  ...
  `nsf_vs_form_d_comparison.parquet`, `nsf_vs_form_d_comparison.md`,
...
- [ ] 2.7 Add security-type / offering-size decomposition view (uses #286's
  ...
  published 1.82x SBIR-to-Form-D leverage ratio scoped to NSF only.
- [ ] 2.8 Add unit + integration tests under `tests/unit/nsf_vc/` and
  `tests/integration/nsf_vc/`. Integration test reuses #286's existing
...
- [ ] X.3 After Phase 2 ships, evaluate whether to extend the same machinery
  to DoD / NIH / DOE (separate spec, not in this scope).
