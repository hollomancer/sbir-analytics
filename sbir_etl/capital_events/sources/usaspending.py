...
def build_usaspending_events(cohort: Iterable[dict], source_path: Path) -> Iterator[dict]:
    ...
    with source_path.open() as f:
        for line in f:
            ...
            yield {
                ...
                "metadata": json.dumps(
                    {
                        "funding_sub_agency": (contract.get("Funding Sub Agency") or "").strip()
                        or None,
                        "awarding_agency": (contract.get("Awarding Agency") or "").strip() or None,
                        "awarding_sub_agency": (contract.get("Awarding Sub Agency") or "").strip()
                        or None,
