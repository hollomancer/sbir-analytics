...
Usage:
    python -m sbir_etl.ucc.export_cohort
    python -m sbir_etl.ucc.export_cohort --form-d-details path/to/form_d_details.jsonl
...
def build_cohort_rows(
    ...
    for rec in form_d_records:
        ...
        agencies = [str(a["agency"]) for a in joined_awards if a.get("agency")]
...
def main() -> int:
    ...
    parser.add_argument("--form-d-details", type=Path, default=data_path("form_d_details.jsonl"))
