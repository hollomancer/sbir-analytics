...
Usage:
    python -m sbir_etl.ucc.cohort_state_filter
"""
    ...
    for row in cohort:
        ...
        try:
            if using_real_lookup:
                record = lookup_ca_sos(name, client=session)
            else:
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=data_path("ucc1_pilot_ca_org_cohort.jsonl"))
    parser.add_argument(
        "--checkpoint", type=Path, default=data_path("ucc1_pilot_state_filter_checkpoint.jsonl")
    )
