...
Epistemic tier: exploratory. The loading and materialization mechanism is
deterministic, but award admission is a contestable analytical policy selected
through a named cohort profile. Outputs are therefore non-citable.
...
COHORT_PROFILE_V1 = "tech-area-cohort-v1"
SUPPORTED_COHORT_PROFILES = frozenset({COHORT_PROFILE_V1})


def load_taxonomy() -> dict[str, dict]:
    raw = read_yaml_mapping(TAXONOMY_PATH, description="CET taxonomy")
...
def load_area_config(area_id: str) -> dict:
    ...
    cfg = read_yaml_mapping(path, description=f"{area_id} cohort profile")
    ...
    profile = cfg.get("cohort_profile")
    if profile not in SUPPORTED_COHORT_PROFILES:
        raise ValueError(
            f"{path} cohort_profile must be one of {sorted(SUPPORTED_COHORT_PROFILES)}, "
            f"got {profile!r}"
        )
    return cfg
...
def aggregate_composition(
    cohort: list[dict],
    censor_year: int = 2023,
    cohort_profile: str = COHORT_PROFILE_V1,
) -> dict:
...
def write_methodology_stub(
    ...
    lines = [
        ...
        f"**cohort_profile:** `{cfg['cohort_profile']}`  ",
        f"**cet_id:** `{cfg.get('cet_id')}`  ",
        "**Status:** exploratory / non-citable  ",
        "**Generated by:** `sbir_etl.reporting.tech_area_cohort`",
        "",
        "## Cohort sizes",
        "",
...
def render_policy_brief_stub(cfg: dict, summary: dict, composition: dict) -> str:
    ...
    lines = [
        ...
        "**Status:** Provisional, exploratory / non-citable; bounded estimates, "
        "not final program rates  ",
...
def materialize_tech_area_cohort(
    area_id: str,
    *,
    awards_csv: Path | None = None,
    sample_negatives: int = 15,
) -> Path:
    """Materialize one named, exploratory technology-area cohort profile.

    The mechanics are deterministic for a fixed award snapshot, area config,
    taxonomy, and optional signal artifacts. The profile still makes
    contestable admission decisions, so callers must preserve the non-citable
    label on its outputs.
    """

    cfg = load_area_config(area_id)
    taxonomy = load_taxonomy()
    awards_csv = awards_csv or DATA / "raw" / "sbir" / "award_data.csv"
    if not awards_csv.exists():
        raise FileNotFoundError(f"awards CSV not found: {awards_csv}")
    ...
    spot = contamination_spotcheck(enriched, core, soft, negatives, sample_negatives)
    print(
    ...
    composition = aggregate_composition(enriched, cohort_profile=cfg["cohort_profile"])
    ...
    summary = {
        "_epistemic": {
            "tier": EPISTEMIC_TIER,
            "citable": False,
            "cohort_profile": cfg["cohort_profile"],
        },
        "area_id": cfg["area_id"],
        "cohort_profile": cfg["cohort_profile"],
...
def main(argv: list[str] | None = None) -> int:
    """CLI adapter for :func:`materialize_tech_area_cohort`."""
    ...
    parser.add_argument(
        "--awards",
        type=Path,
        default=DATA / "raw" / "sbir" / "award_data.csv",
    ...
    try:
        materialize_tech_area_cohort(
            args.area,
            awards_csv=args.awards,
