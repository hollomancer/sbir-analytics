...
class SbirAwardKeyProfile(StrEnum):
    ...
    Profiles are frozen the moment any persisted award key is written under
    them. A change to the hashing logic or field list is a new profile (e.g.
    ``SBIR_SOURCE_V3``), never a modification of an existing one. Editing a
    profile in place would silently invalidate all stored keys without raising
    an error.
    """
