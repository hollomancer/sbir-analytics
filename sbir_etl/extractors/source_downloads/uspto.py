...
USPTO_AI_PATENT_URL = (
    "https://data.uspto.gov/ui/datasets/products/files/ECOPATAI/2023/ai_model_predictions.csv.zip"
)
...
def download_odp_file(
    product_file: str, api_key: str, dest_path: Path, session: requests.Session
) -> dict:
    ...
    if result["size"] < 4096:
        ...
        if not head.startswith(b"PK"):
            binary_match = re.search(rb'https://[^\s"]+', head)
            if binary_match:
                print("   Response was a mint message, not the file — following presigned URL...")
                return stream_download(
                    binary_match.group(0).decode().rstrip("."), dest_path, session
                )
    return result
...
def _stream_response_to_file(response: requests.Response, dest_path: Path) -> dict:
    ...
    with open(dest_path, "wb") as out:
        for chunk in response.iter_content(chunk_size=8 * 1024 * 1024):  # 8MB chunks
            ...
            if content_length:
                ...
                print(
                    f"  {pct:.1f}% ({downloaded / 1024 / 1024:.1f} MB) - {speed:.1f} MB/s", end="\r"
                )
    ...
    if first_bytes[:2] != b"PK":
        ...
        print(
            f"⚠️  Warning: File does not appear to be a ZIP (magic bytes: {first_bytes[:4].hex()})"
        )
  ...
  python scripts/data/download_uspto.py --dataset assignments
    ...
    try:
        ...
        if product_file:
            ...
            result = download_odp_file(
                f"{PATENTSVIEW_PRODUCT}/{patentsview_file}", api_key, dest_path, session
            )
        else:
        ...
        print("Suggestion: Check network connectivity or try again later")
        print("=" * 60)
        sys.exit(1)
        ...
        print(f"URL: {_redact_url(source_url)}")
        print(f"Error: {e}")
        print("Suggestion: Verify URL is correct and accessible")
        print("=" * 60)
        sys.exit(1)
