...
def download_sam_public_extract(dest_dir: Path) -> dict:
    """Download the SAM.gov Public V2 entity snapshot and write it locally.

    Wraps the bulk-extract strategy into a single callable for use by Dagster
    ops and other package consumers. Returns a result dict with ``rows``,
    ``path``, and ``partial`` keys.

    Raises ``ValueError`` if the bulk download returns no validated records.
    """
    df = _download_bulk_extract()
    if df is None or df.empty:
        raise ValueError(
            "SAM.gov Public V2 bulk download returned no validated entity records"
        )
    path = _write_local(df, dest_dir, name=PARQUET_NAME)
