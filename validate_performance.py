...
def create_performance_dataset(num_awards=1000, num_contracts=5000):
    ...
    for i in range(num_contracts):
        contracts.append(FederalContract(
            ...
            competition_type=[CompetitionType.SOLE_SOURCE, CompetitionType.FULL_AND_OPEN][i % 2],
...
def measure_throughput(detector, awards, contracts, target_detections=10000):
    ...
    for i, award in enumerate(awards):
        ...
        if (i + 1) % 100 == 0:
            ...
            print(f"   Processed {i+1:,} awards, {total_detections:,} detections, {rate:,.0f} det/min")
...
def main():
    ...
    if meets_target:
        print(f"   Performance exceeds target by {results['detections_per_minute'] - target_rate:,.0f} det/min")
    else:
    ...
    print(f"   Status: {'✅ EFFICIENT' if memory_mb < 500 else '⚠️ HIGH' if memory_mb < 1000 else '❌ EXCESSIVE'}")
