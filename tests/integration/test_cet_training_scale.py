...
def test_cet_training_scale_synthetic():
    ...
    assert isinstance(train_metrics, dict) or getattr(model, "is_trained", False), "Training did not return metrics or mark model as trained"
    ...
    # Map expectations
    assert primary_ids[0] in ("artificial_intelligence",), f"Unexpected primary for text 0: {primary_ids[0]}"
    assert primary_ids[1] in ("quantum_information_science",), f"Unexpected primary for text 1: {primary_ids[1]}"
    ...
    print(f"[CET SCALE TEST] Trained on {len(X_train)} examples, recall@1 (sample) = {recall_at_1:.3f}")
