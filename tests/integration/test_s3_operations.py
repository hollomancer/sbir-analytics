...
def cleanup_test_files(s3_client, s3_test_bucket, s3_key_prefix):
    """Cleanup test files after each test."""
    yield
    # Cleanup after test
    try:
        response = s3_client.list_objects_v2(Bucket=s3_test_bucket, Prefix=s3_key_prefix)
...
class TestS3Upload:
    ...
    def test_upload_small_file(self, s3_client, s3_test_bucket, s3_key_prefix, tmp_path):
    ...
    def test_upload_binary_file(self, s3_client, s3_test_bucket, s3_key_prefix, tmp_path):
...
class TestS3Download:
    ...
    def test_download_file(self, s3_client, s3_test_bucket, s3_key_prefix, tmp_path):
        """Test downloading a file from S3."""
        # Upload test file
        key = f"{s3_key_prefix}/download_test.txt"
    ...
    def test_resolve_data_path_with_s3(self, s3_client, s3_test_bucket, s3_key_prefix):
        """Test resolve_data_path with real S3 URL."""
        # Upload test file
        key = f"{s3_key_prefix}/resolve_test.txt"
...
class TestS3Fallback:
    ...
    def test_fallback_to_local_when_s3_missing(self, s3_test_bucket, s3_key_prefix, tmp_path):
        ...
        s3_url = f"s3://{s3_test_bucket}/{s3_key_prefix}/nonexistent.txt"
    ...
    def test_prefer_local_over_s3(self, s3_client, s3_test_bucket, s3_key_prefix, tmp_path):
        """Test prefer_local flag uses local even when S3 exists."""
        # Upload to S3
        key = f"{s3_key_prefix}/prefer_test.txt"
...
class TestS3Permissions:
    ...
    def test_can_list_bucket(self, s3_client, s3_test_bucket, s3_key_prefix):
        """Test that we can list objects in the test bucket."""
        # Upload a test file
        key = f"{s3_key_prefix}/list_test.txt"
    ...
    def test_can_delete_objects(self, s3_client, s3_test_bucket, s3_key_prefix):
        """Test that we can delete objects from the test bucket."""
        # Upload a test file
        key = f"{s3_key_prefix}/delete_test.txt"
        s3_client.put_object(Bucket=s3_test_bucket, Key=key, Body=b"test")
