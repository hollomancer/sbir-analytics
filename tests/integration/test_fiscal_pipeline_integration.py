...
def mock_fiscal_naics_enriched(mock_enriched_sbir_awards):
    ...
    df["fiscal_naics_source"] = ["original", "usaspending", "sam_gov", "original", "usaspending"] * 4
    return df
...
class TestFiscalPipelineIntegration:
    ...
    def test_end_to_end_pipeline(
        ...
        prepared_df = prepared_result.value if hasattr(prepared_result, "value") else prepared_result
        ...
        adjusted_df = adjusted_result.value if hasattr(adjusted_result, "value") else adjusted_result
        ...
        wage_sum = sum(float(v) if isinstance(v, (Decimal, int, float)) else 0.0 for v in wage_values)
        ...
        if wage_sum == 0.0:
            ...
            if mock_compute_impacts.called:
                ...
                print(f"DEBUG: mock_compute_impacts last call args: {mock_compute_impacts.call_args}")
        ...
        final_wage_sum = sum(float(v) if isinstance(v, (Decimal, int, float)) else 0.0 
                            for v in impacts_df["wage_impact"].fillna(0.0))
        assert final_wage_sum > 0.0, f"Wage impacts should be non-zero after fix, got sum={final_wage_sum}"
        
        # Step 6: Test tax_base_components
        components_result = fiscal_assets.tax_base_components(context, impacts_df)
        components_df = components_result.value if hasattr(components_result, "value") else components_result
        ...
        summary_result = fiscal_assets.fiscal_return_summary(
            context, tax_df, adjusted_df
        )
        ...
        scenarios_df = scenarios_result.value if hasattr(scenarios_result, "value") else scenarios_result
        assert len(scenarios_df) > 0
        assert "scenario_id" in scenarios_df.columns
        
        # Step 10: Test uncertainty_analysis
        uncertainty_result = fiscal_assets.uncertainty_analysis(
            context, scenarios_df, tax_df
        )
        uncertainty_df = uncertainty_result.value if hasattr(uncertainty_result, "value") else uncertainty_result
        ...
        assert "fiscal_adjusted_amount" in adjusted_df.columns or "inflation_adjusted_amount" in adjusted_df.columns, \
