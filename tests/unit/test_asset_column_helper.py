"""Unit tests for AssetColumnHelper utility."""
...
class TestAssetColumnHelper:
    """Test AssetColumnHelper utility methods."""
    ...
    def test_find_award_id_column_partial_match(self):
    ...
    def test_find_award_id_column_not_found(self):
        """Test when award ID column is not found."""
        df = pd.DataFrame({"name": ["A", "B"], "value": [1, 2]})
        result = AssetColumnHelper.find_award_id_column(df)
        assert result is None

    def test_find_patent_id_column_exact_match(self):
    ...
    def test_find_patent_id_column_partial_match(self):
        """Test finding patent ID column with partial match."""
        df = pd.DataFrame({"grant_number": [1, 2], "title": ["A", "B"]})
        result = AssetColumnHelper.find_patent_id_column(df)
        assert result == "grant_number"

    def test_find_patent_id_column_not_found(self):
        """Test when patent ID column is not found."""
        df = pd.DataFrame({"title": ["A", "B"], "abstract": ["C", "D"]})
        result = AssetColumnHelper.find_patent_id_column(df)
        assert result is None
